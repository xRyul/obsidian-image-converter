...
export class ImageAlignmentManager {
	...
	debouncedValidateNoteCache: Debouncer<
		[notePath: string, noteContent: string],
		Promise<void>
	>;
	...
	addAlignmentOptionsToContextMenu(
		menu: Menu,
				...
				if (file instanceof TFile) {
					if (file.extension === "md") {
						...
						for (const notePath of allNotes) {
							...
							const imageHash = this.getImageHash(
								notePath,
								file.path
							);
							await this.removeImageFromCache(
								notePath,
								imageHash
							); // Pass hash instead of path
						}
					}
			...
			this.app.vault.on("rename", async (file, oldPath) => {
				if (file instanceof TFile) {
					if (file.extension === "md") {
						...
						for (const notePath of allNotes) {
							const oldImageHash = this.getImageHash(
								notePath,
								oldPath
							);
							const newImageHash = this.getImageHash(
								notePath,
								file.path
							);

							if (this.cache[notePath][oldImageHash]) {
								this.cache[notePath][newImageHash] =
									this.cache[notePath][oldImageHash];
	...
	/**
	 * Sets a default alignment for an image only if no alignment already exists.
	 * @param notePath - Path to the note containing the image
	 * @param imageSrc - Source path of the image
	 * @param position - The alignment position to apply
	 * @param wrap - Whether to enable text wrapping (default: false)
	 * @returns true if a new alignment was created, false if one already existed
	 */
	...
	public getRelativePath(imageSrc: string): string {
		...
		const imageFile = this.app.vault
			.getFiles()
			.find((file) => file.path.endsWith(src));
		if (imageFile) {
		...
		try {
			...
			await this.lock.acquire("applyAlignments", async () => {
				const defaultAlignment =
					this.plugin.settings.imageAlignmentDefaultAlignment;
				const allowDefault =
	...
	public async validateNoteCache(notePath: string, noteContent: string) {
		...
		await this.lock.acquire("validateCache", async () => {
			...
			const imageHashes = imageLinks.map((link) =>
				this.getImageHash(notePath, link)
			);
			// console.log("Calculated image hashes:", imageHashes);

			// Find cached images that are no longer in the note
			const imagesToRemove = cachedImages.filter(
				(cachedImageHash) => !imageHashes.includes(cachedImageHash)
			);
	...
	scheduleCacheCleanup() {
		...
		const interval =
			this.plugin.settings.imageAlignmentCacheCleanupInterval;
		if (interval > 0) {
...
function murmurHash3128(key: string, seed: number): string {
	...
	for (let i = 0; i < nblocks; i++) {
		...
		h3 = (h3 << 15) | (h3 >>> 17);
		h3 = (Math.imul(h3, 5) + 0xe6546b64) >>> 0;
		...
		h4 = (h4 << 13) | (h4 >>> 19);
		h4 = (Math.imul(h4, 5) + 0xe6546b64) >>> 0;
	}
	...
	if (tailLen > 0) {
		switch (tailLen) {
			case 15:
				k4 ^= (key.charCodeAt(tailStart + 14) & 0xff) << 16; // fallthrough
			case 14:
				k4 ^= (key.charCodeAt(tailStart + 13) & 0xff) << 8; // fallthrough
			case 13:
				k4 ^= (key.charCodeAt(tailStart + 12) & 0xff) << 0;
				k4 = Math.imul(k4, c1);
				k4 = (k4 << 15) | (k4 >>> 17);
				k4 = Math.imul(k4, c2);
				h4 ^= k4; // fallthrough
			case 12:
				k3 ^= (key.charCodeAt(tailStart + 11) & 0xff) << 24; // fallthrough
			case 11:
				k3 ^= (key.charCodeAt(tailStart + 10) & 0xff) << 16; // fallthrough
			case 10:
				k3 ^= (key.charCodeAt(tailStart + 9) & 0xff) << 8; // fallthrough
			case 9:
				k3 ^= (key.charCodeAt(tailStart + 8) & 0xff) << 0;
				k3 = Math.imul(k3, c1);
				k3 = (k3 << 15) | (k3 >>> 17);
				k3 = Math.imul(k3, c2);
				h3 ^= k3; // fallthrough
			case 8:
				k2 ^= (key.charCodeAt(tailStart + 7) & 0xff) << 24; // fallthrough
			case 7:
				k2 ^= (key.charCodeAt(tailStart + 6) & 0xff) << 16; // fallthrough
			case 6:
				k2 ^= (key.charCodeAt(tailStart + 5) & 0xff) << 8; // fallthrough
			case 5:
				k2 ^= (key.charCodeAt(tailStart + 4) & 0xff) << 0;
				k2 = Math.imul(k2, c1);
				k2 = (k2 << 15) | (k2 >>> 17);
				k2 = Math.imul(k2, c2);
				h2 ^= k2; // fallthrough
			case 4:
				k1 ^= (key.charCodeAt(tailStart + 3) & 0xff) << 24; // fallthrough
			case 3:
				k1 ^= (key.charCodeAt(tailStart + 2) & 0xff) << 16; // fallthrough
			case 2:
				k1 ^= (key.charCodeAt(tailStart + 1) & 0xff) << 8; // fallthrough
			case 1:
				k1 ^= (key.charCodeAt(tailStart + 0) & 0xff) << 0;
				k1 = Math.imul(k1, c1);
				k1 = (k1 << 15) | (k1 >>> 17);
				k1 = Math.imul(k1, c2);
				h1 ^= k1;
	...
	return `${(h4 >>> 0).toString(16).padStart(8, "0")}${(h3 >>> 0)
		.toString(16)
		.padStart(8, "0")}${(h2 >>> 0).toString(16).padStart(8, "0")}${(
		h1 >>> 0
	)
		.toString(16)
		.padStart(8, "0")}`;
